...
Creating the list (and, thus, the list of nodes) is fairly straight-forward. I implemented that Huffman algorithm by having a list of pointers to nodes and then changing the pointers without changing the order of the nodes themselves in memory. So, every time I insert a new node that's pointing to two other nodes on the list, I simply shift the necessary pointers while leaving the nodes in place.

To create the table of values, I simply had each node contain a pointer to its parent and a data value for the type of node it was (whether it was a left of right node, internal or leaf, etc). I then traversed the tree and, when arriving upon a leaf node, find my way back up the tree, writing whether I was coming from a left or right node along the way. This is how the table of values is made.

I tried other methods that were more memory effiecient, but I found that these methods inherently didn't work with all file types. They were also very complex to implement. So, for the sake of human readabilty and expanding to work for all file types, I used this method of parent node pointers.

Encoding the file now that I had a list of codes was the conceptually easier part, but the more technically challenging part. In C, you can't write single bits to a file (in fact, I don't know of a single programming language that allows you to do that, probably some archaic ones). In fact, the smallest unit of data you can write to a fileare bytes (with the fputc() function). My solution to this was to create an `unsigned char` variable to act as a "buffer". In `fileio.c`, I made functions that would write these bits to the buffer instead and then (when the buffer was full) write the buffer to the file. I utilized bit shifting and bit masking to accomplish this goal, and the end result worked surprisingly well with few bugs.
